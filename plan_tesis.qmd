...
Un antecedente interesante es el presentado por @martinsVariationalAutoencodersEvolutionary2022b. Aquí se propone el uso de AVs en el contexto de AGs para optimización de características dirigido al diseño de proteínas (específicamente variantes de Luciferasa bacteriana *luxA*). A tal fin se emplean dos configuraciones de AVs para procesar datos 
Se emplea la base de datos InterPro. Para el proceso , y se propone una dos 

We evaluated different sets of proteins taken from the raw dataset, from sampling and decoding the VAE latent space, and from the sets of proteins optimised using the EAs.

Tal es el caso cuando se lleva a cabo la optimización del diseño de proteínas, donde se deben equilibrar objetivos conflictivos para obtener proteínas funcionales. En el contexto de nuestro experimento, los Algoritmos Evolutivos (EAs, por sus siglas en inglés) optimizan sobre las representaciones del espacio latente del VAE y, por lo tanto, los individuos de los EAs están codificados como vectores reales.



Los desarrollos recientes en el aprendizaje profundo generativo han fomentado nuevos métodos de ingeniería para el diseño de proteínas. Aunque los modelos generativos profundos entrenados en secuencias de proteínas pueden aprender representaciones biológicamente significativas, el diseño de proteínas con propiedades optimizadas sigue siendo un desafío. Combinamos arquitecturas de aprendizaje profundo con computación evolutiva para dirigir el proceso generativo de proteínas hacia conjuntos específicos de propiedades para abordar este problema. El espacio latente de un Autoencoder Variacional se explora mediante algoritmos evolutivos para encontrar los mejores candidatos. Se concibió un conjunto de problemas de un solo objetivo y de múltiples objetivos para evaluar la capacidad de los algoritmos para optimizar proteínas. Las tareas de optimización consideran la hidrofobicidad promedio de las proteínas, su solubilidad y la probabilidad de ser generadas por un perfil de Modelo Oculto de Markov funcional definido. Los resultados muestran que los Algoritmos Evolutivos pueden lograr buenos resultados mientras permiten más variabilidad en el diseño del experimento, lo que resulta en un conjunto mucho mayor de proteínas novedosas posiblemente funcionales.
