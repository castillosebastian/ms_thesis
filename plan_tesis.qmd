...
La aplicación de AVs como técnica de aumentación de datos es extendida fuera del campo de los AGs. Se aplica al tratamiento varios tipos de datos: imágenes [@fajardoOversamplingImbalancedData2021; @aiGenerativeOversamplingImbalanced2023; @khmaissiaConfidenceGuidedDataAugmentation2023; @kwarciakDeepGenerativeNetworks2023], texto [@zhangImproveDiverseText2019] , habla [@blaauwModelingTransformingSpeech2016; @latifVariationalAutoencodersLearning2020] y música [@robertsHierarchicalLatentVector2019], y distintos formatos: tabulares [@leelarathnaEnhancingRepresentationLearning2023], longitudinales [@ramchandranLearningConditionalVariational2022] y grafos [@liuConstrainedGraphVariational2018].

Particularmente su aplicación a problemas de datos desbalanceados o alta dimensionalidad y bajo número de muestras ha mostrado buenos resultados.

\[desarrollar :

En @fajardoOversamplingImbalancedData2021 se investiga si los modelos generativos condicionales profundos (específicamente las variantes condicionales de las redes generativas antagónicas (GANs) y los autoencoders variacionales (AVs)), pueden aprender efectivamente las distribuciones de las clases minoritarias. El artículo busca determinar si estos modelos pueden generar observaciones sintéticas que mejoren el rendimiento de un clasificador. Los resultados indican que efectivamente los modelos generativos profundos superan a los métodos tradicionales (ej. sobremuestreo) en problemas de clasificación, especialmente en casos de desequilibrio severo.

En @aiGenerativeOversamplingImbalanced2023 se ataca el problema de datos desbalanceados y se propone una forma de aumentación de datos en la clase minoritaria que condiciona la generación de muestras a las características de la distribución que tienen los datos de la clase mayoritaria. El método se llama *Majority-Guided VAE* y procura incorporar en la generación información intraclase e interclases, con el fin de propagar la diversidad y riqueza de la mayoría en la minoría, y mitigar así el sobreajuste de los modelos.

En @khmaissiaConfidenceGuidedDataAugmentation2023 se emplean AVs para aumentación de datos en una tarea de clasificación de imágenes con enfoque semi-supervisado. Primero, entrenamos un modelo CNN básico. Luego, identificamos regiones desafiantes en el espacio de características al encontrar todas las muestras mal clasificadas y las muestras correctamente clasificadas con valores de confianza bajos. Estas muestras se utilizan para entrenar un Autoencoder Variacional (VAE). A continuación, el VAE se utiliza para generar imágenes sintéticas. Finalmente, estas imágenes sintéticas se usan junto con las imágenes originales etiquetadas para entrenar un nuevo modelo de manera semi-supervisada. Los resultados empíricos en conjuntos de datos de referencia como STL10 y CIFAR-100 muestran que las muestras sintéticamente generadas pueden diversificar aún más los datos de entrenamiento, lo que lleva a una mejora en la clasificación de imágenes en comparación con los enfoques de línea de base completamente supervisados que utilizan solo los datos disponibles.

En @leelarathnaEnhancingRepresentationLearning2023 se emplean combinaciones (`ensembles`) de AVs para atacar el problema de la reducción de dimensionalidad ante datos de alta dimensionalidad y bajo número de muestras. A través de una serie de experimentos en ocho conjuntos de datos del mundo real, se demuestra que el método aprende mejores representaciones latentes, y conduce a una mayor precisión en una tarea de clasificación.

**AV en el contexto del AG**

En @martinsVariationalAutoencodersEvolutionary2022b .... Los desarrollos recientes en el aprendizaje profundo generativo han fomentado nuevos métodos de ingeniería para el diseño de proteínas. Aunque los modelos generativos profundos entrenados en secuencias de proteínas pueden aprender representaciones biológicamente significativas, el diseño de proteínas con propiedades optimizadas sigue siendo un desafío. Combinamos arquitecturas de aprendizaje profundo con computación evolutiva para dirigir el proceso generativo de proteínas hacia conjuntos específicos de propiedades para abordar este problema. El espacio latente de un Autoencoder Variacional se explora mediante algoritmos evolutivos para encontrar los mejores candidatos. Se concibió un conjunto de problemas de un solo objetivo y de múltiples objetivos para evaluar la capacidad de los algoritmos para optimizar proteínas. Las tareas de optimización consideran la hidrofobicidad promedio de las proteínas, su solubilidad y la probabilidad de ser generadas por un perfil de Modelo Oculto de Markov funcional definido. Los resultados muestran que los Algoritmos Evolutivos pueden lograr buenos resultados mientras permiten más variabilidad en el diseño del experimento, lo que resulta en un conjunto mucho mayor de proteínas novedosas posiblemente funcionales.

**AVEvolutivo**

En @wuEVAEEvolutionaryVariational2023 Desde la perspectiva del cuello de botella de información, los VAE (Autoencoders Variacionales) pueden tratarse como un proceso de compresión de información con pérdida. Ajustar la divergencia KL dentro de un rango controla el cuello de botella de información que fluye desde la representación de variables latentes hasta la reconstrucción de muestras, beneficiando así el equilibrio entre compresión y reconstrucción (Alemi et al. 2017; Burgess et al. 2018). Este cuello de botella de información variacional nos inspira para mejorar el mecanismo y la función objetivo del VAE. Para abordar directamente el equilibrio entre generación e inferencia en un entorno dinámico en evolución sin restricciones en redes y entradas, presentamos un nuevo VAE evolutivo (eVAE). El eVAE incorpora e integra el aprendizaje evolutivo variacional y el cuello de botella de información variacional en el VAE para lograr una mejor exploración óptima y el equilibrio entre la compresión de la representación y el ajuste de la generación. Sin embargo, integrar el aprendizaje evolutivo en el VAE es un tema abierto. Proponemos el algoritmo genético variacional en el eVAE para optimizar los objetivos del VAE y su exploración de modelado de manera evolutiva. En consecuencia, el eVAE optimiza dinámicamente la inferencia del VAE (los términos KL) de manera evolutiva y probabilística, lo que ajusta aún más la generación del VAE hacia el equilibrio entre generación e inferencia.
