...
### Título: "Aumentación de datos mediante autoencoders variacionales y su impacto en el desempeño de estrategias de selección de características basados en algoritmos genéticos."
...
Estos desafíos son abordados por los AGs de manera conveniente y creativa.[^3] Así, su eficacia para tratar datos de grandes dimensiones se funda en su capacidad para explorar el problema dividiéndolo en subespacios de características y, al mismo tiempo, explotar aquellas regiones de mayor valor en cada subespacio [@goldbergdavide.GeneticAlgorithmsSearch1989]. Ambas funciones -exploración y explotación- dependen de operadores estocásticos que permiten al algoritmo reconfigurar el espacio de búsqueda y poner a prueba sus complejas dependencias. Ese procedimiento es orientado por una función de aptitud asociada -generalmente- a un modelo de aprendizaje automático. Dicha función evalúa las distintas posibilidades combinatorias encontradas por el algoritmo en el conjunto de datos observados y retroalimenta el proceso exploratorio. La dinámica completa tiene como resultado un procedimiento experimental de búsqueda capaz de seleccionar las características o atributos de mayor aptitud en una población.

[^3]: Ciertamente, no son sus atributos aislados los que le dan esa posibilidad, sino la interacción de sus componentes.

Dicho lo anterior, no es menos cierto que esta capacidad de los AGs como método de selección de características depende de la evaluación de aptitud que orienta la optimización, y tal evaluación descansa -finalmente- en la disponibilidad de datos. En efecto, tanto la existencia de una población de individuos (datos muestrales), como la diversidad y riqueza de su perfil genético (las propiedades estadísticas de sus atributos), condicionan la función objetivo y por esa vía también a los AGs. La disponibilidad de datos resulta entonces un factor clave no solo cuando se emplea como función objetivo modelos complejos [@alzubaidiSurveyDeepLearning2023], sino también cuando se dispone de conjuntos de datos cuyas clases se encuentran desbalanceadas [@wongUnderstandingDataAugmentation2016, @blagusSMOTEHighdimensionalClassimbalanced2013]. En ambos escenarios, la escasez de datos degrada la capacidad informativa de la función objetivo [@hastieElementStatisticalLearning2009], afectando gravemente el proceso de selección de características.

El desafío que presenta la disponibilidad de datos a los proyectos enfocados en selección de características ha encontrado en las estrategias de **aumentación** un campo de posibles respuestas [@gmComprehensiveSurveyAnalysis2020]. Entre esas respuestas el uso de Autoencoders Variacionales (en adelante AV) aplicados en algoritmos de aprendizaje automático en general ocupa un lugar central.

En efecto, los AVs constituyen modelos generativos (redes neuronales profundas con arquitectura *encoder-decoder*) capaces de aprender una representación latente de datos observados y generar nuevas muestras con las mismas características fundamentales (i.e. similar distribución conjunta de probabilidad) que los datos originales [@kingmaIntroductionVariationalAutoencoders2019]. Esta capacidad generativa, fundada en la aptitud para modelar la distribución de probabilidad de ciertos datos, es particularmente efectiva por el hecho de que prescinde de fuertes supuestos estadísticos a los que adscriben otros modelos y por su *escalabilidad* gracias a emplear *retropropagación* como estrategia de optimización [@kingmaIntroductionVariationalAutoencoders2019]. Hoy los AV son ampliamente utilizados en biología molecular, química, procesamiento de lenguaje natural, astronomía, entre otros [@ramchandranLearningConditionalVariational2022].

Recapitulando, la posibilidad de expandir el conjunto de datos mediante el uso de AV abre nuevas alternativas para afrontar el problema de la selección de características y disponibilidad de datos en el contexto de los AGs. Estas alternativas no solo parecen prometedoras como estrategias de crecimiento poblacional previo a la implementación del proceso de optimización evolutiva, sino incluso como partes funcionales de sus operaciones de selección.[^4] De este modo, la integración de ambas tecnologías ofrece un enfoque provechoso para abordar el problema de selección de características en escenarios de alta dimensionalidad y muestras escasas. A la fecha de publicación de la presente no hemos encontrado experiencias publicadas de aplicación general de AVs en el ámbito de selección de características con AGs. En la medida que esto sea así creemos que nuestro aporte a la comunidad de investigadores y practicantes de la disciplina estará en proveer información y experimentación sobre la combinación de estos algoritmos. Dicho aporte tendría un alcance nacional a todos aquellos equipos dedicados al problema de selección de características aplicando computación evolutiva.

[^4]: Así, la integración de los AVs en el contexto de los AGs podría dirigirse no solo al aumento de datos, sino también a la multiplicación de ciertos subconjuntos de características valiosas
...
La escasez de datos muestrales afecta el proceso de selección de características aplicando AGs debido a su impacto negativo en la función objetivo. Este impacto es particularmente grave en escenarios de alta dimensionalidad y muestras escasas. Por eso, la técnica de aumentación de datos mediante AVs plantea una posible solución, ofreciendo distintas alternativas de implementación.
...
2.  Integrar estrategias de aumentación de datos basadas en AV en los AGs.

3.  Evaluar el desempeño de las estrategias propuestas empleando conjuntos de datos de diferente complejidad.

4.  Comparar el desempeño de AGs aumentados mediante las estrategias desarrolladas frente a implementaciones sin aumentación.
...
[^9]: <http://clopinet.com/isabelle/Projects/NIPS2003/>
...
Detallamos a continuación los requerimientos técnicos necesarios para la implementación del trabajo propuesto, utilizando algoritmos genéticos y autoencoders variacionales. Se detallan las librerías de Python que serán utilizadas para el desarrollo, así como los conjuntos de datos en los que se llevarán a cabo los experimentos. Además, se describen las arquitecturas computacionales que se explorarán en el estudio. 
...
2. **Leukemia**
    - Fuente: Artículo Golub et al., 1999. El conjunto de datos puede encontrarse en varios repositorios de investigación en biología computacional.

3. **GCM**
    - Fuente: Artículo Ramaswamy et al., 2001. Disponible en repositorios de biología computacional.
