...
### Título: "Aumentación de datos mediante autoencoders variacionales y su impacto en el desempeño de estrategias de selección de características basadas en algoritmos genéticos."
...
Estos desafíos son abordados por los AGs de manera conveniente y creativa.[^4] Así, en el marco de este algoritmo cada individuo (muestra) representa una solución candidata, con un perfil genético particular determinado por un subconjunto de características. La búsqueda de las mejores soluciones comienza con una población inicial de subconjuntos de características generados aleatoriamente. Estos subconjuntos se evalúan utilizando una función de aptitud, y los individuos con mejor rendimiento (puntaje) son seleccionados para la reproducción. Este proceso continúa durante un cierto número de generaciones hasta que se cumple una condición de terminación.

[^4]: Ciertamente, no son sus atributos aislados los que le dan esa posibilidad, sino la interacción de sus componentes.

La eficacia para tratar datos de grandes dimensiones se funda, precisamente, en su capacidad para explorar el problema dividiéndolo en subespacios de características y, al mismo tiempo, explotar aquellas regiones de mayor valor en cada subespacio [@goldbergdavide.GeneticAlgorithmsSearch1989]. Ambas funciones -exploración y explotación- permiten al algoritmo reconfigurar el espacio de búsqueda y poner a prueba sus complejas dependencias. Como vimos, ese procedimiento es orientado por una función de aptitud, asociada -generalmente- a modelos de aprendizaje automático **\[citar ejemplos en footnote\]**, que evalúa las distintas posibilidades combinatorias encontradas por el algoritmo y retroalimenta el proceso exploratorio. La dinámica completa tiene como resultado un procedimiento experimental de búsqueda capaz de seleccionar las características de mayor aptitud en una población.

Dicho lo anterior, no es menos cierto que esta capacidad de los AGs depende de la evaluación de aptitud que orienta la selección, y tal evaluación descansa -finalmente- en la disponibilidad de datos. En efecto, tanto la existencia de una población de individuos (datos muestrales), como la riqueza de su perfil genético (propiedades estadísticas de sus atributos), condicionan la función objetivo y por esa vía también al proceso de selección de características mediante AGs. La disponibilidad de datos resulta así un factor clave. Este aspecto del algoritmo, consecuencia del uso de modelos de aprendizaje automático como función objetivo, no solo se presenta cuando se utilizan modelos complejos (multitud de parámetros) que demandan cantidad creciente de datos para entrenamiento [@alzubaidiSurveyDeepLearning2023]. Sino también surge cuando se trabaja sobre datos cuyas clases se encuentran desbalanceadas [@blagusSMOTEHighdimensionalClassimbalanced2013], que suponen una insuficiencia parcial de datos. En ambos escenarios, la falta de información de entrenamiento degrada la capacidad informativa de la función objetivo [@hastieElementStatisticalLearning2009], afectando gravemente el proceso de selección de características.

En esa línea, el desafío que presenta la disponibilidad de datos a los proyectos enfocados en selección de características ha encontrado en las estrategias de **aumentación** un campo de posibles respuestas [@gmComprehensiveSurveyAnalysis2020]. Entre esas respuestas el uso de Autoencoders Variacionales (en adelante AV) aplicados a distintos problemas de aumentación ocupa un lugar preponderante [@ramchandranLearningConditionalVariational2022].

En efecto, los AVs constituyen modelos generativos[^5] capaces de aprender una representación latente de datos observados y generar nuevas muestras con las mismas características fundamentales[^6] que los datos originales [@kingmaIntroductionVariationalAutoencoders2019]. Esta capacidad generativa, fundada en la aptitud para modelar la distribución de probabilidad de datos observados es particularmente efectiva por el hecho de que prescinde de fuertes supuestos estadísticos a los que adscriben otros modelos. Al mismo tiempo, esa efectividad está dada también por su *escalabilidad*, basada en el empleo de *retropropagación* como estrategia de optimización [@kingmaIntroductionVariationalAutoencoders2019]. Hoy los AVs son ampliamente utilizados en biología molecular, química, procesamiento de lenguaje natural, astronomía, entre otros [@ramchandranLearningConditionalVariational2022].

[^5]: Redes neuronales profundas con arquitectura *encoder-decoder* [@kingmaIntroductionVariationalAutoencoders2019].

[^6]: Similar distribución conjunta de probabilidad.

Por todo lo visto hasta aquí advertimos que la posibilidad de expandir el conjunto de datos mediante el uso de AVs abre nuevas alternativas para afrontar el problema de la selección de características aplicando AGs. Estas alternativas no solo parecen prometedoras como estrategias orientadas a la multiplicación de muestras de entrenamiento para mejorar el desempeño de la función objetivo, sino incluso como partes funcionales de sus operaciones de selección.[^7] De este modo, la integración de ambas tecnologías ofrece un enfoque provechoso para abordar el problema de selección de características en distintos escenarios que enfrentan los AGs.

[^7]: Así, la integración de los AVs en el contexto de los AGs podría dirigirse no solo a la multiplicación general de datos, sino también a la multiplicación selectiva de ciertos subconjuntos de características valiosas

A la fecha de publicación del presente trabajo no hemos encontrado experiencias publicadas de aplicación general de AVs en el ámbito de selección de características mediante AGs. En la medida que esto sea así creemos que nuestro aporte a la comunidad de investigadores y practicantes de la disciplina estará en proveer información y experimentación sobre la combinación de estos algoritmos. Dicho aporte tendría un alcance nacional a todos aquellos equipos dedicados al problema de selección de características aplicando computación evolutiva.
...
[^12]: <http://clopinet.com/isabelle/Projects/NIPS2003/>
